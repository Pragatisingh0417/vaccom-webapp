...
const brandLinks = [
  ...
  { img: "/brand-image/i-Vac.webp", slug: "i-Vac" }, // ✅ Correct casing
...
export default function Navbar() {
  ...
  const handleDropdown = (name: "brands" | "products" | "stores") => setOpenDropdown(name);
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-8">
            ...
            <ul className="hidden md:flex gap-6 items-center text-[18px] relative">
              ...
              >
                <div className="flex items-center gap-1">Brands <FaAngleDown className="text-xs" /></div>
                ...
                {openDropdown === "brands" && (
                  <div className="absolute left-0 top-14 rounded-b-lg bg-white text-black shadow-lg z-10 w-[700px] px-4 py-4 flex gap-2 flex-wrap transition-all duration-200">
                    {brandLinks.map((brand, index) => (
                      <Link key={index} href={`/product-brand/${toSlug(brand.slug)}`}>
                        <img src={brand.img} alt={brand.slug} className="w-30 h-15 object-contain cursor-pointer hover:scale-105 transition" />
                      </Link>
                    ))}
                  </div>
                )}
              ...
              <li
                ...
                className="relative cursor-pointer hover:text-white px-3 py-4 hover:bg-black"
              >
                <div className="flex items-center gap-1">Products <FaAngleDown className="text-xs" /></div>
                ...
                {openDropdown === "products" && (
                  <div className="absolute top-full left-0 bg-white text-black shadow-lg z-10 w-[300px] px-4 py-4 flex flex-col gap-2 transition-all duration-200">
                    {productLinks.map((product) => (
                      <Link key={product.slug} href={`/product-category/${toSlug(product.slug)}`} className="flex items-center gap-2 hover:text-red-600 transition duration-200">
                        <img src={product.img} alt={product.name} className="w-10 h-6 object-contain" />
              ...
              <li
                ...
                className="relative cursor-pointer hover:text-white px-3 py-4 hover:bg-black"
              >
                <div className="flex items-center gap-1">Stores <FaAngleDown className="text-xs" /></div>
                {openDropdown === "stores" && (
                  <div className="absolute top-full left-0 bg-white text-black shadow-xl z-10 py-2 px-4 w-48 space-y-2">
                    {storeLinks.map((city) => (
                      <Link key={city} href={`/our-stores/${toSlug(city)}-store`} className="block hover:text-red-600 transition">{city}</Link>
                    ))}
                  </div>
                )}
              </li>
          ...
          <div className="md:hidden bg-white px-4 py-2 space-y-3 text-[15px] text-black">
            ...
            <div>
              ...
              <ul className="ml-4 mt-1 flex flex-wrap gap-3">
                {brandLinks.map((brand, index) => (
                  <li key={index}>
                    <Link href={`/product-brand/${toSlug(brand.slug)}`}>
                      <img src={brand.img} alt={brand.slug} className="w-16 h-16 object-contain cursor-pointer hover:scale-105 transition" />
                    </Link>
                  </li>
                ))}
              </ul>
            ...
            <div>
              ...
              <ul className="ml-4 mt-1 space-y-2">
                {productLinks.map((product) => (
                  <li key={product.slug}>
                    <Link href={`/product-category/${toSlug(product.slug)}`} className="flex items-center gap-2 hover:text-red-600 transition duration-200">
                      <img src={product.img} alt={product.name} className="w-10 h-6 object-contain" />
                      <span>{product.name}</span>
                    </Link>
                  </li>
            ...
            <div>
              ...
              <ul className="ml-4 mt-1 space-y-1">
                {storeLinks.map((city) => (
                  <li key={city}>
                    <Link href={`/our-stores/${toSlug(city)}-store`}>{city}</Link>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="flex items-center border rounded overflow-hidden bg-white text-black mt-2">
              <input type="text" placeholder="Search Products" className="px-3 py-2 text-[15px] w-full outline-none" />
              <button className="bg-black px-4 py-2 text-white"><FaSearch /></button>
            </div>
          </div>
        )}
      </nav>
